---
name: jumpserver-automation
description: Automate Jumpserver Linux asset login through ssh Jumpserver, totp jumpserver, and bundled Expect/Python scripts, then execute an explicit command on the selected server with production-risk checks. Use when the user asks to initialize automated Jumpserver access, validate Jumpserver automation prerequisites, select a Jumpserver asset, log in through Jumpserver, or run a command on a Linux server via Jumpserver.
---
...
```bash
alias jump="/absolute/path/to/jumpserver-automation/scripts/login_jump.exp"
```
